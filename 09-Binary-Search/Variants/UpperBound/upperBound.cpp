...
int main()
{
    ...
    cout << upper_bound(arr1.begin(), arr1.end(), val) - arr1.begin();
    cout << endl;

    // using upper_bound() to check if val exists
    // in arr2, multiple occurrence, prints 4
    cout << upper_bound(arr2.begin(), arr2.end(), val) - arr2.begin();
    cout << endl;

    // using upper_bound() to check if val exists
    // in arr3, no occurrence,  prints 2
    // ( index of next higher)
    cout << upper_bound(arr3.begin(), arr3.end(), val) - arr3.begin();
    cout << endl;
    return 0;
}